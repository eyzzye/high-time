...
#define HT_THIS_CLASS_NAME chaser
...
chaser::chaser(ht::enemy::grade grade_val):
    enemy(ht::object::type::ENEMY, (int)ht::object::enemy_id::CHASER),
    _grade(grade_val),
    ...
    _rect_ptr(NULL),
    _trace(NULL),
    _sprite(bn::sprite_items::chaser.create_sprite(0, 0)),
...
static bn::fixed chaser_wait_time = 10.0f;
static bn::fixed chaser_move_time = 30.0f;
void chaser::update() {
    ...
    if (_ai_stat.current_act_stat == ai_action_stat::INIT) {
        ...
        _ai_stat.param = &chaser_wait_time;
...
void chaser::set_position(bn::fixed_point& pos) {
    _rect_ptr->set_position(pos);
}

void chaser::set_ai_scenario(struct ai_scenario_t* ai_list, int size) {
    _ai_scenario.clear();
...
void chaser::_set_tiles(int index) {
    switch (_grade) {
    case ht::enemy::grade::NORMAL:
        _sprite.set_tiles(
            bn::sprite_items::chaser.tiles_item().create_tiles(index));
        break;
    case ht::enemy::grade::HARD:
        _sprite.set_tiles(
            bn::sprite_items::chaser.tiles_item().create_tiles(index));
        _sprite.set_palette(
            bn::sprite_items::chaser_hard.palette_item());
        break;
    case ht::enemy::grade::INSANE:
        _sprite.set_tiles(
            bn::sprite_items::chaser.tiles_item().create_tiles(index));
        _sprite.set_palette(
            bn::sprite_items::chaser_insane.palette_item());
        break;
    default:
...
void chaser::_set_target_enable(bool on) {
    if (_target_enable == on) return;
    if (on == false) _target_sprite->set_visible(false);
    _target_enable = on;
}

void chaser::collision_handler_func(ht::collision_manager::rect_body* body_a, ht::collision_manager::rect_body* body_b) {
#if 1
    BN_ASSERT(body_a != NULL, "chaser::collision_handler_func: body_a == NULL");
    BN_ASSERT(body_b != NULL, "chaser::collision_handler_func: body_b == NULL");
#else
    ...
    if (object_a->get_type() == ht::object::type::ENEMY) {
        //BN_LOG("chaser::collision_handler_func(): ", object_b->get_type());
    } else {
        //BN_LOG("chaser::collision_handler_func(): ", object_a->get_type());
    }
#endif
}
